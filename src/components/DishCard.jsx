...
const DishCard = ({ dish, day, index, isAdminMode, onEdit, onDelete }) => {
  ...
  return (
    ...
    >
      ...
      {(dish.images && (dish.images[0] || dish.images[1])) && (
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
          {dish.images[0] && (
            <div className="aspect-video rounded-md overflow-hidden border border-border">
            <img  className="w-full h-full object-cover" alt={`Imagen 1 de ${dish.name}`} src={image1Src} />
            </div>
          )}
          {dish.images[1] && (
             <div className="aspect-video rounded-md overflow-hidden border border-border">
              <img  className="w-full h-full object-cover" alt={`Imagen 2 de ${dish.name}`} src={image2Src} />
            </div>
          )}
        </div>
      )}
